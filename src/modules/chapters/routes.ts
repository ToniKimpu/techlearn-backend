...
router.post(
  "/chapters",
  requirePermission("chapter:write"),
  validate({ body: createChapterBody }),
  async (req, res, next) => {
    try {
...
router.put(
  "/chapters/:id",
  requirePermission("chapter:write"),
  validate({ params: idParam, body: updateChapterBody }),
  async (req, res, next) => {
    try {
      const { title, sortOrder, imageUrl, label, content, teacherGuide, subjectId } =
        req.body as z.infer<typeof updateChapterBody>;
      const updated = await chapterService.update(BigInt(req.params.id as string), {
        title,
        sortOrder,
        imageUrl,
        label,
