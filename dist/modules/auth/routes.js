...
import { cacheSession, generateRefreshToken, getCachedSession, getSessionExpiry, removeAllCachedSessions, removeCachedSession, } from "../../utils/session.js";
...
router.post("/auth/login", authLimiter, validate({ body: loginBody }), (req, res, next) => {
...
router.post("/auth/logout", validate({ body: logoutBody }), async (req, res, next) => {
    try {
...
router.post("/auth/refresh-token", validate({ body: refreshTokenBody }), async (req, res, next) => {
    try {
        const { refreshToken } = req.body;
        // Try Redis first, fall back to DB
        let sessionData = await getCachedSession(refreshToken);
        if (!sessionData) {
